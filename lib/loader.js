...
// dep loaders
const styleLoaderPath = normalize.dep('vusion-style-loader')
...
module.exports = function (content) {
  ...
  if (isProduction && options.cssMinimize !== false) {
  ...
  if (parts.styles.length) {
    ...
    parts.styles.forEach((style, i) => {
      ...
      const hasVueStyleLoader = requireString.indexOf('vusion-style-loader') > -1
      // vusion-style-loader exposes inject functions during SSR so they are
      // always called
      const invokeStyle =
      ...
      if (moduleName) {
        ...
        if (moduleName in cssModules) {
          ...
          if (!needsHotReload) {
            styleInjectionCode += invokeStyle(`this['${moduleName}'] = cssModules['${moduleName}']`)
          } else {
            ...
            styleInjectionCode +=
              invokeStyle(`cssModules['${moduleName}']`) +
  ...
  if (!isProduction) {
    ...
    output +=
      'Component.options.__moduleId = ' + JSON.stringify(moduleId) + '\n'
  }
  ...
  function getRawLoaderString (type, part, index, scoped) {
    ...
    let styleCompiler = ''
    const midStyleLoader = midLoaders[lang] ? ensureBang(midLoaders[lang]) : ''
    if (type === 'styles') {
      ...
      styleCompiler =
        ...
        '?' +
        JSON.stringify({
          // a marker for vusion-style-loader to know that this is an import from a vue file
          vue: true,
          id: moduleId,
          scoped: !!scoped,
    ...
    if(type === 'styles') {
      if(loader instanceof Array) {
        loader = loader.map((item) => {
          if(item.loader === 'vusion-style-loader'){
            if(!item.options)
              item.options = {};
            item.options.attrs = part.attrs;
          }
          return item;
        })
      } else if(typeof loader === 'object') {
        if(loader.loader === 'vusion-style-loader'){
          if(!loader.options)
            loader.options = {};
          loader.options.attrs = part.attrs;
        }
      } else if(typeof loader === 'string') {
        let loaders = loader.split('!');
        const regx = new RegExp(`^vusion-style-loader`,'g');
        loaders = loaders.map((item) => {
          if(regx.test(item)) {
            const parts = item.split('?');
            const options = parts[1]?JSON.parse(parts[1]): {};
            options.attrs = part.attrs;
            parts[1] = JSON.stringify(options);
            return parts.join('?');
          }
          return item
        });
