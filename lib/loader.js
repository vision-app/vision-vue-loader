...
module.exports = function (content) {
  ...
  var babelOptions = hasBabel && options.babel ? '?' + JSON.stringify(options.babel) : ''
  var bubleOptions = hasBuble && options.buble ? '?' + JSON.stringify(options.buble) : ''
  var defaultLoaders = {
    html: templateCompilerPath + '?id=' + moduleId,
    css: (isServer ? '' : styleLoaderPath + '?insertAt=byId!') + 'css-loader' + (needCssSourceMap ? '?sourceMap' : ''),
    js: hasBuble ? ('buble-loader' + bubleOptions) : hasBabel ? ('babel-loader' + babelOptions) : ''
  }
  ...
  function getLoaderString (type, part, index, scoped) {
    ...
    var rewriter = type === 'styles' ? styleRewriter + (scoped ? '&scoped=true!import-global-loader!' : '!import-global-loader!') : ''
  ...
  if (Object.keys(cssModules).length) {
    ...
    exports +=
      ...
      'Object.keys(__vue_styles__).forEach(function (key) {\n' +
        ...
        'if (__vue_options__.computed[key])\n' +
        '  module = Object.assign(__vue_options__.computed[key](), module)\n' +
